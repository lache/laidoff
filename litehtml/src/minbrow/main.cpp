...
int main() {
	...
	browser_context.load_master_stylesheet(_t(R"asdf(
html {
    display: block;
    height:100%;
    width:100%;
...
article, aside, footer, header, hgroup, nav, section 
{
	...
	const int w = 1080;
	const int h = 1920;
	std::shared_ptr<litehtml::text_container> container(new litehtml::text_container(w, h));
	
	std::ifstream t("HTMLPage1.html");
	std::string str((std::istreambuf_iterator<char>(t)), std::istreambuf_iterator<char>());
	auto wstr = utf8_to_wstring(str);

	auto doc = litehtml::document::createFromString(wstr.c_str(), container.get(), &browser_context);
