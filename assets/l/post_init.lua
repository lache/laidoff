...
function on_ui_event(id)
	...
	elseif id == 'tutorial_button' then
		...
		start_coro(function()
			lo.puck_game_set_tutorial_guide_str(c.puck_game, '왼쪽 스틱으로 플레이어를 움직이세요.')
			yield_wait_ms(1500)
			lo.puck_game_set_tutorial_guide_str(c.puck_game, '잘 했습니다.')
			yield_wait_ms(1500)
			lo.puck_game_set_tutorial_guide_str(c.puck_game, '이제 하얀색 공 근처까지 가 보세요.')
			yield_wait_ms(1500)
			lo.puck_game_set_tutorial_guide_str(c.puck_game, '잘 했습니다.')
			yield_wait_ms(1500)
			lo.puck_game_set_tutorial_guide_str(c.puck_game, '공에 부딪치면 공이 파란색으로 바뀝니다.')
			yield_wait_ms(1500)
